...
class Network:
    ...
    def __init__( self, hidden=1, batchSize=0, inputData=None, outputData=None,
                  ...
                  desiredData=None, classification=False):
        self.hidden = hidden
        ...
        self.outputLayer = Layer.OutLayer( size=outputDim, alpha=0.2 )
        self.hiddenLayers = [ Layer.HiddenLayer( size=hiddenDim, alpha=0.2 ) ]
    ...
    def pointsForPlot(self):

        # prediction on training data
        for i in range(len(self.inputData)):
            xk = utils.unScale(self.test(self.inputData[i])[0], self.outScale)
            self.stepErr[3].append(xk)

        # prediction on Test data
        for i in range(len(self.testData)):
            xk = utils.unScale(self.test(self.testData[i])[0], self.outScale)
            self.stepErr[4].append(xk)
    ...
    def getAccuracy(self, step):
        ...
        for i in range(len(self.inputData)):
            xk = utils.unScale( self.test( self.inputData[i] )[ 0 ], self.outScale )
            avgErr += ( xk - utils.unScale( self.outputData[i][0], self.outScale ) )**2

        # Calculating the average of training data error
        avgErr /= len(self.inputData)
        # print(avgErr)
        if step < 30000:
